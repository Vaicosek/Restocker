...
- `/item edit item [coin] [stackable] [stack_size]` — (manager) fix an item's price or
  stackability. `/item info` shows price, stock, barrel size and stackability.
  `/item edit` sets price (add `per_stack:true` to quote per stack of 64), stackability
  and `worker_cost` — it absorbed `/item_set_price`. `/item add` adds a catalog item.
...
- If an item is wrongly marked stackable, its barrel size (and therefore fullness and
  restock quantity) is 64× off — fix with `/item edit … stackable:False`.
