...
DEF = dict(
    ...
    max_auction_days=14.0,       # HARD deadline from starts_at — anti-snipe can never push past it
...
def _coin_amount(v):
    ...
    NaN slips past every ordinary money guard in this file because every comparison
    against NaN is False: `amt < min_bid` does not reject it and `bal < amt` does not
    reject it. `json.loads` accepts a bare `NaN` token, so a partner server can push one
    all the way through the satellite relay, and the only thing that stops it today is
    `int(float('nan'))` raising — one line PAST two guards that both failed open.
    Callers turn a None from here into a normal refusal instead of an exception."""
    try:
        f = float(v)
    except (TypeError, ValueError):
        return None
...
# ── H1 — claim-first settlement ───────────────────────────────────────────────────────
# `_finalize_sale_core` used to pay the seller and the house in transactions of their own
# BEFORE marking the listing sold, and `auction_sweep_loop` re-runs every 60 seconds over
# `status='active' AND ends_at <= now`. One `database is locked` on that final UPDATE —
# routine contention with the web server on its own OS thread against busy_timeout=5000,
# not a crash — and the sweep settles the same listing again a minute later, and again.
# On an 8.5M listing that is 8.5M minted per minute. The row is now CLAIMED in one atomic
# UPDATE before a single coin moves, and each money step inside the claim carries its own
# coin_ledger key so a retry after a partial failure pays only what has not already paid.
_SETTLING = "settling"      # transient claim state — not 'active', so the sweep skips it
_STALE_CLAIM_MINUTES = 10   # a claim older than this is assumed dead and re-armed


def _claim_listing_for_settlement(_db, listing_id: int) -> bool:
    """One atomic `UPDATE ... WHERE status='active'`. True only if WE won the row.

    The owner's rule: mark the row first, act only if the mark stuck. `update_land_listing`
    has no WHERE clause beyond the id, so the conditional claim is written here against the
    same `db()` helper it uses — same connection, same pragmas, no new plumbing."""
    with _db.db() as conn:
        cur = conn.execute(
            "UPDATE land_listings SET status=?, updated_at=datetime('now') "
            "WHERE id=? AND status='active'", (_SETTLING, int(listing_id)))
        return cur.rowcount == 1


def _release_listing_claim(_db, listing_id: int) -> bool:
    """Put a claimed-but-unfinished listing back to 'active' so the sweep retries it.

    This is what makes a failed settlement RECOVERABLE rather than silently unpaid: the
    listing goes back in the queue instead of sitting sold-to-nobody. Releasing is safe
    only because every money step inside the claim is separately guarded by its own
    coin_ledger key — the retry re-checks each key and skips whatever already paid."""
    try:
        with _db.db() as conn:
            cur = conn.execute(
                "UPDATE land_listings SET status='active', updated_at=datetime('now') "
                "WHERE id=? AND status=?", (int(listing_id), _SETTLING))
            return cur.rowcount == 1
    except Exception as e:
        # Worst case: the row stays 'settling'. Nobody is double-paid (that was the whole
        # point) but nobody is paid either until it is re-armed — which _rearm_stale_claims
        # below does automatically on the next sweep, and this line tells a human how.
        log.error("[realestate] STUCK SETTLEMENT CLAIM on #%s — release failed: %s. No coins "
                  "were double-paid. It re-arms itself within %s minutes; to force it: "
                  "UPDATE land_listings SET status='active' WHERE id=%s AND status='settling';",
                  listing_id, e, _STALE_CLAIM_MINUTES, listing_id)
        return False


def _rearm_stale_claims(_db) -> int:
    """Re-arm settlement claims that were never released — the recovery half of H1.

    A claimed listing is invisible to `get_expired_active_listings()`, so a process death
    between the claim and the release would strand it forever. `_finalize_sale_core` is
    fully synchronous and sub-second, so anything still claimed after _STALE_CLAIM_MINUTES
    is dead by definition. Re-entry is safe: the coin_ledger keys make every money step in
    the settlement idempotent."""
    try:
        with _db.db() as conn:
            cur = conn.execute(
                "UPDATE land_listings SET status='active', updated_at=datetime('now') "
                "WHERE status=? AND updated_at <= datetime('now', ?)",
                (_SETTLING, f"-{int(_STALE_CLAIM_MINUTES)} minutes"))
            n = int(cur.rowcount or 0)
        if n:
            log.warning("[realestate] re-armed %s stale settlement claim(s) for retry", n)
        return n
    except Exception as e:
...
def _ledger_has_charge(_db, user_id, reason: str) -> bool:
    """H2 — fail-OPEN idempotency probe, for the DEBIT side only.

    `_db.coin_ledger_has` deliberately fails CLOSED (returns True when it cannot read),
    which is the right bias for a PAYOUT — if we can't verify, never pay again. It is
    exactly the wrong bias for a CHARGE: a false "already charged" would hand out a plot
    for free, which mints. So the charge side gets its own probe that fails OPEN. If we
    cannot prove the buyer was already debited, we debit. The bad case is then a double
    CHARGE (recoverable by hand, and both attempts are visible in coin_ledger under the
    same reason) rather than a free sale (not recoverable, and it creates coins)."""
    try:
        with _db.db() as conn:
            row = conn.execute(
        ...
        log.warning("[realestate] charge-idempotency probe failed for %s / %s: %s",
                    user_id, reason, e)
        return False
...
def _finalize_sale_core(listing_id: int, buyer_id, price: float) -> dict:
    ...
    Claim-first (H1): the listing is flipped 'active' -> 'settling' in ONE atomic UPDATE
    before any coin moves, and money moves only if that UPDATE won the row. On any raise
    the claim is released so the sweep retries; each payout inside carries its own
    coin_ledger key, so the retry pays only what has not already paid."""
    import Restocker_db as _db
    ...
    # H3 — a NaN/inf/non-positive price must be a refusal here, not a ValueError six lines
    # down inside int(round(...)) after the caller has already collected the money.
    price = _coin_amount(price)
    if price is None:
        return {"ok": False, "error": "That sale price isn't a valid coin amount."}
    ...
    if not _claim_listing_for_settlement(_db, listing_id):
        return {"ok": False, "error": "That listing is no longer active."}
    try:
        # Each payout below carries its own coin_ledger key. `coin_ledger_has` fails CLOSED
        # (True when it cannot read), so under DB stress it refuses to pay a second time —
        # the correct bias for money leaving the house.
        ...
        # `record_coin_ledger` already writes this exact key on every seller payout — it has
        # always been a perfect idempotency key and nothing read it. Now something does.
        ...
        # H1: gated on the same key as the payout, so a retry cannot farm points either —
        # and loyalty tiers CUT the commission, so a re-settle loop used to mint a little
        # more on every pass.
        ...
        # whatever already paid will not pay again on the retry.
...
def _place_bid_core(listing_id: int, bidder_id, amount=None) -> dict:
    ...
    # H3 — if `reserve`/`current_bid` ever went non-finite, min_bid is non-finite too and
    # EVERY comparison below is False, i.e. the listing would accept any amount from
    # anyone. (SQLite stores a NaN REAL as NULL, but it stores `inf` faithfully, and
    # `_min_next_bid` then raises OverflowError inside round().) Refuse the listing rather
    # than either trade against a poisoned floor or blow up on the caller.
    try:
    ...
    # H3 — validate the user-supplied amount at the entry point. NaN is truthy and passes
    # `amt < min_bid` and `bal < amt` alike; a bare NaN token also survives json.loads and
    # therefore the satellite hop from a partner server. Refuse it like any other bad input.
    if amount:
    ...
    if listing.get("ends_at"):
        ...
        if (end_ts - now_ts) < anti_snipe * 60.0:
            # H4 — the extension is now BOUNDED. It used to reset ends_at to now + N with no
            # cap and no maximum auction length, so two colluding accounts (A bids, B outbids
            # — A refunded in full — A outbids, forever) could hold an auction open
            # indefinitely at zero net cost and no genuine bidder could ever be last.
            # A hard deadline at starts_at + max_auction_days ends that: extensions still
            # work, they just cannot push past the wall. Integer epoch seconds throughout.
...
def _instant_buy_core(listing_id: int, buyer_id) -> dict:
    ...
    if not listing or listing["status"] != "active":
        return {"ok": False, "error": "That listing isn't active."}
    # H3 — a non-finite or non-positive buy_now is "no instant-buy price", not a raise.
    price = _coin_amount(listing.get("buy_now"))
    if not price:
    ...
    # H2 — instant-buy had no idempotency at all. The compensating refund below only fires
    # on a returned {"ok": False}, which _finalize_sale_core only produced BEFORE any money
    # moved; any raise after that skipped it entirely, so the buyer was debited, the seller
    # was paid, _record_network_land_buy caught the exception and told the buyer "try again
    # shortly" — and nothing stopped the retry from buying the same plot a second time.
    # One key per (buyer, listing) closes that: a retry re-uses the first charge.
    ...
    try:
        ...
        # would mint. Tell the truth, and let the retry be safe rather than cheap — the
        # charge key above means it costs the buyer nothing to click again.
        ...
        return {"ok": False, "listing_id": listing_id, "price": float(price),
                "error": ("Your payment went through and this purchase is still completing. "
                          ...
                          "hasn't completed, tell a manager listing "
                          f"#{listing_id}.")}
    if res.get("ok"):
...
def create_listing_core(seller_id, title, starting_price, buy_now=None, details=None,
    ...
    # H3 — `NaN <= 0` is False, so a NaN starting price used to sail through the check
    # below and become the listing's reserve, i.e. the opening bid _min_next_bid returns.
    if _coin_amount(starting_price) is None:
    ...
    if buy_now not in (None, ""):
        ...
        if _coin_amount(bn) is None:                                    # H3
            return {"ok": False, "error": "Buy-Now price must be > 0."}
...
class LandExchangeCog(commands.Cog):
    ...
    @app_commands.describe(
        ...
        max_auction_days="Hard cap on total auction length — anti-snipe can never extend past it",
    ...
    async def config(self, interaction: discord.Interaction, commission_pct: Optional[float] = None,
                     ...
                     max_auction_days: Optional[float] = None):
        if not is_manager(interaction):
            return await interaction.response.send_message("⛔ Managers only.", ephemeral=True)
        import Restocker_db as _db
        for key, val in (("commission_pct", commission_pct), ("listing_fee", listing_fee),
                         ...
                         ("max_auction_days", max_auction_days)):
            if val is not None:
                _db.set_config(f"realestate:{key}", str(float(val)))
