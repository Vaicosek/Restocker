...
#: FAIRNESS.md §12.4 layer 3. Same reason as the three above: a database that
#: already has 001–003 still needs `wallet_flags` and `treasury:games`.
ROUND4_MIGRATION_ID: str = "ledger_v2_004_wallet_flags_games_treasury"
...
-- FAIRNESS.md §12.4 layer 3 — "no credit for gambling", as an invariant rather
-- than a convention between two bots.
--
-- Osentar writes `gambling_blocked` here in the SAME core transaction that mints
-- a loan into the borrower's wallet (`adjust(set_flag=...)`), and clears it when
-- their debt reaches zero. `ledger_v2.place_hold` refuses any hold from a service
-- in `GAMBLING_BLOCKED_SERVICES` for a flagged wallet, so a borrower cannot
-- reserve a stake and therefore cannot bet. The rule lives with the coins, and
-- holds even if the games bot is compromised or mis-deployed.
--
-- Deliberately NOT a foreign key to `balances`: `_ensure_wallet` runs first on
-- every write path here, and a flag that could not be written because a wallet
-- row was missing would fail OPEN, which is the one direction this table must
-- never fail. `set_at` is ISO-8601 UTC text, like every other timestamp v2
-- writes.
CREATE TABLE IF NOT EXISTS wallet_flags (
    user_id  TEXT NOT NULL,
    flag     TEXT NOT NULL,          -- 'gambling_blocked'
    set_by   TEXT NOT NULL,          -- the service that wrote it: 'osentar'
    reason   TEXT NOT NULL,          -- shown to the player in the refusal
    set_at   TEXT NOT NULL,
    PRIMARY KEY (user_id, flag)
);

-- The hot path is `place_hold` asking "is THIS user flagged", which the primary
-- key already serves. This index serves the other direction — the ops panel
-- listing everyone currently blocked, which is how staff notice a flag that
-- should have been cleared.
CREATE INDEX IF NOT EXISTS idx_wallet_flags_flag
    ON wallet_flags(flag, user_id);
"""
        ...
        if dry_run:
            missing_tables = [
                t for t in ("ledger_holds", "ledger_idempotency", "ledger_entries",
                            "ledger_meta", "ledger_migrations", "wallet_flags")
