...
#: Write a restriction onto somebody else's wallet. Held by `osentar` alone.
#: Deliberately NOT held by `games`, and that is the whole point of the design:
#: `gambling_blocked` restricts games, so games must not be able to set, clear or
#: even be trusted to read it. A check inside the thing being restricted is not a
#: restriction. See `set_wallet_flag`.
SCOPE_FLAG = "wallet.flag"
...
#:
#: `games` is the same shape as `estates` and for the same reason, plus one more:
#: it is the only service subject to `GAMBLING_BLOCKED_SERVICES`, so it holds
#: neither `wallet.mint` (it cannot conjure a bankroll) nor `wallet.flag` (it
#: cannot lift its own restriction). It needs `wallet.transfer` for the
#: treasury→winner payout leg, and `hold.*` for the two-sided escrow.
SERVICE_SCOPES: dict[str, frozenset[str]] = {
    "osentar": frozenset({SCOPE_READ, SCOPE_TRANSFER, SCOPE_MINT, SCOPE_STOCKS,
                          SCOPE_HOLD, SCOPE_FLAG}),
    "estates": frozenset({SCOPE_READ, SCOPE_TRANSFER, SCOPE_HOLD}),
    "games":   frozenset({SCOPE_READ, SCOPE_TRANSFER, SCOPE_HOLD}),
}
...
TOKEN_ENV: dict[str, str] = {
    ...
    "games":   "LEDGER_TOKEN_GAMES",
}

#: ══ FAIRNESS.md §12.4 layer 3 — the credit-to-gambling rule, as an invariant ══
#:
#: A player must never be able to borrow from Osentar and stake it. Osentar and
#: games are two processes with two databases talking over HTTP, so any version
#: of this rule that lives in either of them is a CONVENTION: it holds until a
#: deploy is skipped, a token is reused, an endpoint times out and somebody
#: fails open, or the games bot is rewritten by somebody who never read the
#: document. The same argument as LEDGER_API_v2.md §5.1, one level up.
#:
#: So the rule lives where the coins live. Osentar writes `gambling_blocked` onto
#: the wallet in the SAME core transaction that mints the loan (`adjust`'s
#: `set_flag`), and `place_hold` refuses every hold a service in this set asks
#: for on a flagged wallet. A player with an outstanding loan cannot reserve a
#: stake, so there is no bet — regardless of what any satellite believes.
#:
#: WHY A SET AND NOT `service == "games"`: a second gambling surface added later
#: is one line here rather than a search for every place the rule was spelled.
#: WHY `place_hold` AND NOT `capture`: escrow is where the money is committed,
#: and it is the FIRST money step of a bet. Refusing at capture would refuse
#: after the player's coins were already reserved, which is the worse failure.
#: WHAT IT DOES NOT DO: it does not touch holds that already exist. See
#: `_assert_not_gambling_blocked`.
GAMBLING_BLOCKED_SERVICES: frozenset[str] = frozenset({"games"})

#: The flags a caller may write. A closed set: an unknown flag is a typo, and a
#: typo that silently becomes a row is a restriction nobody is enforcing.
WALLET_FLAGS: frozenset[str] = frozenset({"gambling_blocked"})
...
def _assert_not_gambling_blocked(conn: sqlite3.Connection, user_id: str,
                                 service: str) -> None:
    """FAIRNESS.md §12.4 layer 3, enforced by the process that owns the coins.

    Refuses a NEW reservation for a service in `GAMBLING_BLOCKED_SERVICES` when
    the wallet carries `gambling_blocked`. `409 gambling_blocked`, which is in
    `games_db.DEFINITE_REFUSAL_CODES` — core evaluated it and provably moved
    nothing, so the caller may treat the bet as refused rather than unknown.

    WHAT THIS DELIBERATELY DOES NOT DO — an existing hold is untouched.

    The flag is checked at the moment a reservation is CREATED and nowhere else.
    A hold that already exists when Osentar disburses a loan stays open, and its
    capture and release still work. That is a decision, not an oversight:

      * the player's stake is already reserved and the house's counter-escrow is
        already reserved against it. Killing the hold mid-round would strand a
        settled outcome with no money behind it — the exact shape of red-team §4,
        where one refusal at the wrong step locked both escrows for an hour.
      * the borrowed coins cannot be in that hold: the hold predates the loan.
        Blocking it would refuse a bet made entirely with the player's own money.
      * holds are short (`MAX_HOLD_SECONDS`) and self-expiring, so the window is
        bounded and visible, and the very next bet is refused.

    The rule is therefore "no new gambling escrow while you owe Osentar", and
    that is exactly what it says on the tin.
    """
    if service not in GAMBLING_BLOCKED_SERVICES:
        return
    row = conn.execute(
        "SELECT reason, set_by, set_at FROM wallet_flags "
        "WHERE user_id = ? AND flag = 'gambling_blocked'",
        (str(user_id),),
    ).fetchone()
    if row is None:
        return
    raise LedgerError(
        "gambling_blocked", 409,
        f"{user_id} is flagged by {row['set_by']}: {row['reason']} "
        f"(set {row['set_at']}). Coins cannot be reserved for {service} until "
        f"the flag is cleared.")


def _assert_flag_authority(service: str) -> None:
    """`wallet.flag`, checked HERE and not only at the handler.

    Every other scope in this module is enforced by `@_require` on the way in,
    and that is enough for them because the handler is the only door. This one
    gets a second check inside the function, deliberately, for one reason: the
    thing it restricts is a service in this same process family, and the failure
    mode being designed against is somebody calling `clear_wallet_flag("games",
    ...)` from a script, a REPL or a future in-process caller that never passes
    through `_require`. A restriction a restricted party can lift is not a
    restriction, so the check goes where the write is.
    """
    if not has_scope(service, SCOPE_FLAG):
        raise LedgerError("forbidden_scope", 403,
                          f"service '{service}' lacks '{SCOPE_FLAG}' — it may not "
                          f"write wallet flags")


def set_wallet_flag(service: str, user_id: str, flag: str, reason: str,
                    *, conn: Optional[sqlite3.Connection] = None) -> dict[str, Any]:
    """Write a restriction onto a wallet. `wallet.flag` scope only.

    Idempotent by primary key: setting a flag that is already set refreshes the
    reason and the timestamp and reports `changed: False`. That matters because
    a borrower with two loans must not need two flags, and repaying one of them
    must not clear the other's block — the flag is a state of the WALLET, and
    Osentar decides that state from its own books each time it changes them.

    `conn` lets a caller put this in an existing transaction; `adjust(set_flag=)`
    does exactly that, so the coins and the restriction land together or not at
    all. Osentar disbursing a loan without the flag, because the second HTTP call
    failed, is the whole leak this closes.
    """
    _assert_flag_authority(service)
    flag = str(flag)
    if flag not in WALLET_FLAGS:
        raise LedgerError("bad_flag", 400,
                          f"unknown flag {flag!r}; known: {sorted(WALLET_FLAGS)}")
    if not (reason or "").strip():
        raise LedgerError("missing_reason", 400,
                          "a flag with no reason is a flag nobody can clear safely")
    uid = str(user_id)

    def _work(c: sqlite3.Connection) -> dict[str, Any]:
        _ensure_wallet(c, uid)
        existed = c.execute(
            "SELECT 1 FROM wallet_flags WHERE user_id=? AND flag=?",
            (uid, flag)).fetchone() is not None
        c.execute(
            "INSERT INTO wallet_flags (user_id, flag, set_by, reason, set_at) "
            "VALUES (?,?,?,?,?) "
            "ON CONFLICT(user_id, flag) DO UPDATE SET "
            "  set_by=excluded.set_by, reason=excluded.reason, set_at=excluded.set_at",
            (uid, flag, str(service), str(reason)[:200], _iso(_utcnow())))
        _record(c, service=service, action="flag_set", user_id=uid, delta=0,
                reason=f"{flag}: {reason}"[:200])
        return {"user_id": uid, "flag": flag, "set": True, "changed": not existed,
                "set_by": str(service), "reason": str(reason)[:200]}
...
def clear_wallet_flag(service: str, user_id: str, flag: str, reason: str = "",
                      *, conn: Optional[sqlite3.Connection] = None) -> dict[str, Any]:
    """Remove a restriction. `wallet.flag` scope only.

    `changed` is False when there was nothing to clear, and that is a 200, not an
    error: Osentar calls this on every repayment that reaches zero debt, and the
    second call in a retry must not fail. Clearing is recorded in
    `ledger_entries` even when it was already clear, because "who lifted this and
    when" is the audit question, and FAIRNESS.md §12.4's own honest list ends at
    "anyone who can edit the database can clear a flag; the trail is what makes
    ...
    def _work(c: sqlite3.Connection) -> dict[str, Any]:
        cur = c.execute("DELETE FROM wallet_flags WHERE user_id=? AND flag=?",
                        (uid, flag))
        _record(c, service=service, action="flag_clear", user_id=uid, delta=0,
                reason=f"{flag}: {reason or 'cleared'}"[:200])
        return {"user_id": uid, "flag": flag, "set": False,
...
def get_wallet_flags(user_id: str) -> dict[str, Any]:
    """Every flag on a wallet. A read; no write lock.

    This is what a blocked player's page renders — the reason and who set it, so
    the refusal names the bank rather than saying "no".
    """
    rows = _conn().execute(
        "SELECT flag, set_by, reason, set_at FROM wallet_flags WHERE user_id=? "
        "ORDER BY flag", (str(user_id),)).fetchall()
    return {"user_id": str(user_id),
            "flags": [dict(r) for r in rows],
            "gambling_blocked": any(r["flag"] == "gambling_blocked" for r in rows)}
...
def place_hold(service: str, user_id: str, amount: int, reason: str,
    ...
    with _tx() as conn:
        _ensure_wallet(conn, uid)
        _assert_not_frozen(conn, uid)
        # FAIRNESS.md §12.4 layer 3. Before the availability test, because a
        # blocked player must be told they are blocked, not that they are broke.
        _assert_not_gambling_blocked(conn, uid, service)
        cur = conn.execute(
...
def capture_hold(service: str, hold_id: str, amount: Optional[int] = None,
    ...
    which is why a satellite without that scope must always pass one.

    `to_user` MAY BE ANY WALLET, including a player's: the lines below do
    `_ensure_wallet(dst)` then `_credit(dst, want)` with no restriction. The one
    constraint lives in the handler (`h_hold_capture`) and it is narrower than
    this docstring used to claim: a destination starting with `treasury:` must be
    the calling service's OWN treasury. This sentence previously read "and it
    must be its own treasury" and `games` read it as a prohibition on paying a
    winner in one atomic capture, which cost it a two-call payout with an
    unreserved gap in the middle. Do not re-narrow it without narrowing the code.
    """
    with _tx() as conn:
        row = _hold_row(conn, hold_id)
...
def adjust(service: str, user_id: str, amount: int, reason: str,
           ...
           *, idem: Optional[_Idem] = None,
           set_flag: Optional[str] = None, clear_flag: Optional[str] = None,
           flag_reason: str = "") -> dict[str, Any]:
    ...
    `set_flag` / `clear_flag` (additionally `wallet.flag`) EXIST FOR ONE CALLER
    AND ONE SENTENCE IN FAIRNESS.md §12.4: "Osentar sets `no_gambling` in the
    same transaction as a loan disbursement". Osentar's books are in `bank.db`
    and the coins are in `restocker.db` over HTTP, so there is no transaction
    spanning both — but the disbursement itself is ONE core transaction, and this
    puts the flag inside it. The alternative, two calls, has a window: mint
    succeeds, flag call fails, and the borrower is holding fresh credit with a
    clean wallet. That window is the entire finding.

    Only the money fields are fingerprinted for idempotency (`FINGERPRINT_FIELDS`
    keeps `adjust` at `user_id`/`amount`), which is correct here: the key is
    `loan-{id}-disburse`, so the same key always carries the same flag, and a
    retry replays the stored response without re-running either half.
    """
    amt = int(amount)
    if amt == 0:
    ...
    if set_flag and clear_flag:
        raise LedgerError("bad_request", 400,
    ...
    with _tx() as conn:
        ...
        if set_flag:
            flag_result = set_wallet_flag(service, uid, set_flag,
                                          flag_reason or reason, conn=conn)
        elif clear_flag:
            flag_result = clear_wallet_flag(service, uid, clear_flag,
                                            flag_reason or reason, conn=conn)
...
async def h_adjust(request: Any) -> Any:
    ...
    set_flag = str(body.get("set_flag") or "").strip() or None
    clear_flag = str(body.get("clear_flag") or "").strip() or None
    # Writing a restriction onto a wallet is a second authority, so it needs a
    # second scope even on an endpoint the caller already holds. `games` has
    # neither, and could not reach this handler in any case.
    if (set_flag or clear_flag) and not has_scope(request["service"], SCOPE_FLAG):
        return _err("forbidden_scope", 403,
                    f"service '{request['service']}' lacks '{SCOPE_FLAG}'")
    with _idempotent(request, "adjust", body) as slot:
        result = adjust(request["service"], uid, int(amount or 0), reason,
                        ...
                        set_flag=set_flag, clear_flag=clear_flag,
                        flag_reason=str(body.get("flag_reason") or "").strip())
        slot["body"] = _ok(result)
    return _json(slot["body"], slot["status"])


@_require(SCOPE_FLAG)
async def h_wallet_flag(request: Any) -> Any:
    """Set or clear a wallet flag on its own — the repayment and staff paths.

    No idempotency key: this endpoint is idempotent by construction (the flag is
    a state, not a movement, and the primary key is the enforcement), which is
    the same reason `LEDGER_API_v2.md` §6 exempts `/hold/extend`. Repeat it as
    often as you like; `changed` tells you whether this call was the one that
    moved it.
    """
    body = await _body(request)
    uid = str(body.get("user_id") or "").strip()
    ...
    if action not in ("set", "clear"):
        return _err("bad_request", 400, "action must be 'set' or 'clear'")
    if action == "set":
        result = set_wallet_flag(request["service"], uid, flag, reason)
    else:
        result = clear_wallet_flag(request["service"], uid, flag, reason)
    return _json(_ok(result))


@_require(SCOPE_READ)
async def h_wallet_flags(request: Any) -> Any:
    """Read a wallet's flags. Any service may read; only `wallet.flag` may write.

    `games` reads this to render WHY a bet is refused — the bank's name, the
    reason and the date — rather than a bare 409. Reading is not enforcing: the
    enforcement is in `place_hold`, and it happens whether or not anyone looked.
    """
    uid = str(request.query.get("user_id") or "").strip()
    if not uid:
        return _err("missing_user_id", 400)
    return _json(_ok(get_wallet_flags(uid)))
...
async def _v1_adjust(request: Any) -> Any:
    ...
    # FAIRNESS.md §12.4 layer 3. Osentar has NOT migrated to v2 — it talks to
    # `/api/v1/bank/adjust` through `restocker_client.py` — so the flag has to be
    # reachable on this alias too, or "written inside the disbursement
    # transaction" is a sentence about a call Osentar cannot make until the
    # migration lands. The v1 caller is always `osentar`, which holds
    # `wallet.flag`; `adjust` re-checks that anyway.
    try:
...
async def _v1_wallet_flag(request: Any) -> Any:
    """v1 alias for `POST /wallet/flag` — the paths with no coin movement.

    Collections take repayment out of SAVINGS, which never touches the wallet, so
    the moment a borrower's debt reaches zero there is no `adjust` for the clear
    to ride along with. Without this route that borrower stays blocked until they
    happen to make a wallet transaction, which is a silent, indefinite punishment
    for having repaid.
    """
    body = await _body(request)
    uid = str(body.get("user_id") or "").strip()
    ...
    action = str(body.get("action") or "set").strip().lower()
    if action not in ("set", "clear"):
        return _err("action must be 'set' or 'clear'.", 400)
    if action == "set":
        result = set_wallet_flag("osentar", uid, flag, reason)
    else:
        result = clear_wallet_flag("osentar", uid, flag, reason)
    return _json({"ok": True, **result})
...
V1_ROUTES: tuple[tuple[str, str, Callable], ...] = (
    ...
    ("post", "/wallet/flag", _v1_wrap(_v1_wallet_flag)),
)
...
V2_ROUTES: tuple[tuple[str, str, Callable], ...] = (
    ...
    ("get", "/holds", h_holds),
    ("post", "/wallet/flag", h_wallet_flag),
    ("get", "/wallet/flags", h_wallet_flags),
    ("get", "/stocks", h_stocks),
