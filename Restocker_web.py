...
async def _handle_api_investor(request):
    ...
    companies, dividends = [], {}
    try:
        _mkts = db.get_public_markets() or {}
    except Exception as e:
    ...
    for mid in _mkts:
        ...
        # frontend showed "Could not load investor data" with every panel stuck.
        try:
            listing = db.get_market_shares(mid) or {}
            ...
            try:
                hist = db.get_dividend_history(mid, 24) or []
            except Exception:
                hist = []
            dividends[mid] = hist
            companies.append({
            ...
            import traceback, sys
            print(f"⚠️  /api/investor: skipping market {mid!r}: {e!r}", file=sys.stderr)
            traceback.print_exc()
            continue
...
def _land_body_fields(body):
    try:
        ...
        lid = 0
    # requester_id accepted as a fallback so a satellite that sends ONLY requester_id
    # (the /cancel and /auction_close shape) still resolves an acting user.
    uid   = str(body.get("bidder_id") or body.get("buyer_id")
                or body.get("requester_id") or "").strip()
    uname = str(body.get("bidder_name") or body.get("buyer_name") or "member").strip()[:64]
    gid   = str(body.get("source_guild_id") or "").strip()
    return lid, uid, uname, gid


def _land_manager_ok(user_id) -> bool:
    """Is this Discord user a V TECH manager? Decided HERE, from OUR roles, in OUR home
    guild — never from the request body.

    The satellite computes `is_manager` from `manage_guild` in the PARTNER guild
    (RestockerLightWeight/app.py cancel_cmd / auction_close_cmd), i.e. a server the owner
    does not control, so any manager claim arriving over the wire is worthless as
    authorization. Mirrors Restocker_main.is_manager() and keeps the HOME_GUILD_ID pin
    that _config_home_ok() already uses for the same reason ("an admin of any other
    server the bot joined couldn't re-point them at themselves").

    Fails CLOSED: any lookup error, missing cache or unready bot returns False.
    Runs on the bot loop (cache reads) — call it via run_on_bot_loop."""
    try:
    ...
    try:
        import os as _os
        import Restocker_main as _m
        if uid in (getattr(_m, "MANAGER_DM_IDS", None) or []):
            return True
        home = int(_os.getenv("HOME_GUILD_ID", "954487497411403806") or 0)
        guild = _m.bot.get_guild(home) if home else None
        ...
        return bool(names & {getattr(_m, "MANAGER_ROLE_NAME", "Manager"),
                             getattr(_m, "MANAGER_ROLE_ALT", "Admin")})
    except Exception as e:
        print(f"⚠️ land manager check failed for {user_id}: {e}")
        return False


async def _land_require_manager(body):
    """Gate for the manager-only land endpoints (/close, /config writes). Returns
    (requester_id, None) when the acting user is a V Tech manager, else
    (requester_id, error_response). Fails CLOSED — no identity, no action."""
    rid = str(body.get("requester_id") or body.get("actor_id") or "").strip()
    if not rid:
        return "", web.json_response(
            {"ok": False, "error": "requester_id is required — update the V Tech Lands & "
                                   "Auctions satellite to the matching build."}, status=400)
    import Restocker_main as _m
    try:
        ok = await _m.run_on_bot_loop(_land_manager_ok, rid)
    except Exception as e:
        return rid, web.json_response(
            {"ok": False, "error": f"Manager check unavailable — try again shortly. ({e})"},
            status=503)
    if not ok:
        return rid, web.json_response(
            {"ok": False, "error": "Managers only — this is decided by your roles on the "
                                   "V Tech home server, not on this one."}, status=403)
    return rid, None
...
async def _handle_network_land_bid(request):
    ...
    try:
        ...
        amount = None
    # NaN/Infinity survive float() and json.loads(), and every comparison against NaN is
    # False — so `amt < min_bid` and `bal < amt` in _place_bid_core BOTH fail open.
    # Reject at the boundary; None still means "bid the minimum".
    import math as _math
    if amount is not None and not _math.isfinite(amount):
        return web.json_response({"ok": False, "error": "That bid amount isn't a number."})
    ...
    if res.get("ok"):
        try:
            ...
            if loop is not None:
                import asyncio as _a
                # int() TRUNCATES: a 1000.6 bid deducted 1001 and this said 1000. Report
                # the number that actually moved — int(round()) matches the deduction at
                # cogs/land_exchange.py _place_bid_core.
                note = (f"💰 Network bid on **#{lid}**: `{int(round(float(res.get('amount') or 0))):,}` 🪙 "
...
async def _handle_network_land_cancel(request):
    ...
    import Restocker_main as _m
    # body["is_manager"] is IGNORED. The satellite derives it from manage_guild in the
    # PARTNER guild, so trusting it let any partner-server admin cancel V Tech listings.
    # Seller-cancels are unaffected (cancel_listing_core allows the seller regardless).
    try:
        is_mgr = await _m.run_on_bot_loop(_land_manager_ok, uid)
    except Exception:
        is_mgr = False
    try:
    ...
    if res.get("ok"):
        try:
            ...
            if loop is not None:
                import asyncio as _a
                _a.run_coroutine_threadsafe(
                    _m._notify_network_land(lid, f"🚫 Listing **#{lid}** cancelled by <@{uid}>."), loop)
        except Exception:
            pass
    return web.json_response(res)
...
async def _handle_network_land_close(request):
    ...
    # This endpoint force-settles a live auction or refunds a standing bid — it moves
    # real coins and it previously carried NO requester identity at all, gated only by
    # manage_guild in the partner guild. Identity is now required and manager is decided
    # on our side. Fails closed.
    rid, deny = await _land_require_manager(body)
    if deny is not None:
        return deny
    import Restocker_main as _m
    try:
...
async def _handle_network_land_config(request):
    ...
    # A WRITE here sets commission_pct globally (0 before an alt's sale, 100 to confiscate
    # the next seller's proceeds). It had no identity check whatsoever. Reads are left
    # open so the deployed satellite's config VIEW keeps working — a read moves no coins.
    if updates:
        rid, deny = await _land_require_manager(body)
        if deny is not None:
            return deny
